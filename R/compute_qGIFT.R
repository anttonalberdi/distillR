...
compute_qGIFT <- function(definition, gene_count_table) {
  ...
  if (grepl(".", definition, fixed = TRUE)) {
    gene_count_table <- gene_count_table %>%
      rowwise() %>%
      mutate(Annotation = list(map_chr(Annotation, function(x) str_replace_all(x, "\\.", "_")))) %>%
      ungroup()
  ...
  definition_quantitative_list <- as.list(rep(definition, ncol(gene_count_table) - 1))
  ...
  for (level in rev(levels_list[[1]])) {
    for (s in names(definition_quantitative_list)) {
      definition_quantitative_list[[s]] <- distillq_definition(sample = s, definition_quantitative_list[[s]], def_table_list[[s]], level, gene_count_table %>% select(Annotation, s))
      if (level != "L0_group") {
